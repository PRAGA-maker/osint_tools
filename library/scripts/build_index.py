...
def collect(base, list_fields):
    ...
    for path, fm, _body in C.iter_skills(base):
        fm = dict(fm)
        # Derive the enrichment flag from the authoritative signal — presence of the
        # authored '## When to use' body — rather than trusting the source field, which
        # drifts (files get flagged 'full' without ever being authored). This keeps the
        # DB, CSV, coverage report and STATUS all counting the same reliable signal.
        if "enrichment" in fm:
            fm["enrichment"] = "full" if "## When to use" in (_body or "") else "stub"
...
def gen_group_indexes(base, tree):
    ...
    for dirpath, dirs, files in os.walk(base):
        ...
        if os.path.exists(idx):
            # Refresh auto-generated nav so it stays in sync after tools are added or
            # pruned; never touch a curated _index.md (one without the generated footer).
            existing = open(idx, encoding="utf-8").read()
            if "Auto-generated by build_index.py" not in existing:
                continue
...
def coverage_report(tools, strategies):
    ...
    out.append("_\"full\" = authored (has a `## When to use` body); the enrichment field is "
               "derived from that signal, not self-reported._\n")
...
def write_status(tools, strategies):
    """Regenerate library/STATUS.md from live counts so it can never go stale by hand."""
    ...
    if os.path.exists(sp):
        src = json.load(open(sp, encoding="utf-8"))
    nsrc = len(src.get("sources", []))
    out = [
        f"# OSINT Skill Library — Status ({date.today().isoformat()})",
        "",
        ...
        "|---|---|",
        f"| **Tools** | **{total}** |",
        f"| — full (authored, has `## When to use` body) | {full} ({100 * full // total if total else 0}%) |",
        f"| — stub (backlog) | {total - full} |",
        f"| **Strategies** | {len(strategies)} |",
        f"| **Sources tracked** (`ledger/sources.json`) | {nsrc} |",
        "",
        "**Tools by MP relevance:** " + " · ".join(f"{k}: {v}" for k, v in mp.most_common()),
        "",
        "**Validation:** run `python library/scripts/validate.py` (enforced in CI). "
        "This file makes no standing 'green' claim — check the badge / CI run.",
        "",
        "See `ledger/coverage.md` for per-category depth and source exhaustion.",
    ]
    with open(os.path.join(C.LIB, "STATUS.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(out) + "\n")
...
def main():
    ...
    write_status(tools, strategies)
    full = sum(1 for t in tools if t.get("enrichment") == "full")
    print(f"indexed tools={len(tools)} ({full} authored) strategies={len(strategies)} "
          f"| group-indexes written={g1 + g2}")
