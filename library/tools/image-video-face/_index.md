...
- **`alos`** (low) — Use when you have a `geolocation` and want Japanese-satellite earth-observation imagery — returns ALOS optical/radar scenes of the area (via
...
- **`amireal`** (low) — Use when you have a face `image` and suspect it is an AI/GAN-generated fake (e.g. ThisPersonDoesNotExist) — returns a manual verdict via an 
...
- **`aperislove`** (medium) — Use when you have an `image` and want to pull hidden data and embedded metadata — returns `metadata-exif` (EXIF/GPS) plus any steganographic
...
- **`cleanup-pictures`** (low) — Use when you have an `image` and want to remove distracting objects/watermarks/text so a cleaner crop reverse-searches better — returns a re
...
- **`fdupes`** (low) — Use when you have a large set of downloaded images/files and want to find byte-for-byte duplicates locally — a CLI de-duplicator for cleanin
