...
- **`waybackpack`** (medium) — Use when you have a `domain`/URL and want its entire Wayback history offline — downloads every archived snapshot so you can diff a site's pa
- **`web-archive-ru`** (medium) — Use when a `domain`/URL isn't well covered by mainstream archives and you want an alternative snapshot source — a Russian web-archive servic
