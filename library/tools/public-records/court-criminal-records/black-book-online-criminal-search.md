---
id: black-book-online-criminal-search
name: Black Book Online - Criminal Search
description: Broad public records aggregation
url: https://www.blackbookonline.info/
category: public-records
path:
- public-records
- court-criminal-records
bestFor: Broad public records aggregation
input: Name, location, record type
output: Criminal records, property, court filings
selectorsIn: []
selectorsOut: []
status: live
pricing: free
opsec: passive
opsecNote: Not FCRA compliant; accuracy not guaranteed
humanInLoop: false
humanInLoopReason: []
bestInteractionPattern: web-manual
trust: unverified
trustNote: ''
missingPersonsRelevance: medium
coverage: []
auth: none
api: false
localInstall: false
registration: false
invitationOnly: false
deprecated: false
relatedTools: []
aliases: []
tags: []
source: arf-seed
lastVerified: ''
enrichment: stub
---

# Black Book Online - Criminal Search

> **Stub** — seeded from OSINT-Framework (`arf-seed`). Body not yet authored.
> Enrich per `schema/templates/tool.template.md`, then set `enrichment: full`.

- **URL:** https://www.blackbookonline.info/
- **Best for:** Broad public records aggregation
- **Input → Output:** Name, location, record type → Criminal records, property, court filings
- **OpSec:** passive. Not FCRA compliant; accuracy not guaranteed

_To enrich:_ verify `trust` & `missingPersonsRelevance`, set `selectorsIn/Out` and `bestInteractionPattern`, write the How-to and Gotchas, link overlaps in `relatedTools`.
