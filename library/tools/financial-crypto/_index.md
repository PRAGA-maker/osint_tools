...
- **`a-ton-of-privacy`** (low) — Use when you have a Telegram `username`/`phone` (+888) or a `.ton` domain and want the owner's TON wallet, balance, scam status and NFTs — r
...
- **`coinwink-com`** (low) — Use when you're tracking a crypto asset relevant to a case and want to be alerted on price moves without watching charts — returns email/Tel
...
- **`matbea`** (low) — Use when you have a Bitcoin `crypto-wallet` address and want to know which exchange, service, or company controls it — returns attributed ow
...
- **`wallet-labels`** (low) — Use when you have a `crypto-wallet` address and want a human-readable label (exchange, whale, scammer, DAO, contract) for it — returns entit
