---
id: email-search-tool-by-inteltechniques
name: Email Search Tool by IntelTechniques
description: 'OSINT tool: Email Search Tool by IntelTechniques.'
url: https://inteltechniques.com/osint/email.search.html
category: email
path:
- email
bestFor: ''
selectorsIn:
- email
selectorsOut:
- name
- social-profile
- phone
status: unknown
pricing: free
opsec: unknown
opsecNote: ''
humanInLoop: false
humanInLoopReason: []
bestInteractionPattern: web-manual
trust: unverified
trustNote: ''
missingPersonsRelevance: high
coverage: []
auth: none
api: false
localInstall: false
registration: false
aliases: []
tags:
- email
source: metaosint
lastVerified: ''
enrichment: stub
---

# Email Search Tool by IntelTechniques

> OSINT tool: Email Search Tool by IntelTechniques.

- **URL:** https://inteltechniques.com/osint/email.search.html
- **Best for:** —
- **Source:** harvested from `metaosint`

MetaOSINT category: Identity / Person Search / Email; recommended by 6 curated OSINT source list(s).

_Enrichment: stub. If stub, complete per `schema/templates/tool.template.md`._
