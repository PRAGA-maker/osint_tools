---
id: carrier-route
name: Carrier Route
description: Lookup tool that provides a list of all carrier routes within a specified radius from a central location
url: http://www.melissadata.com/lookups/cartradius.asp
category: search-engines
path:
- search-engines
bestFor: ''
selectorsIn: []
selectorsOut: []
status: unknown
pricing: free
opsec: passive
opsecNote: ''
humanInLoop: false
humanInLoopReason: []
bestInteractionPattern: web-manual
trust: unverified
trustNote: ''
missingPersonsRelevance: medium
coverage:
- global
auth: none
api: false
localInstall: false
registration: false
aliases: []
tags:
- toddington
- curated-directory
- specialty-search
source: toddington-resources
lastVerified: ''
enrichment: stub
relatedTools:
- melissa-us
- melissadata-campaign-contributions
- mellssa-data
---

# Carrier Route

> Lookup tool that provides a list of all carrier routes within a specified radius from a central location

- **URL:** http://www.melissadata.com/lookups/cartradius.asp
- **Best for:** —
- **Source:** harvested from `toddington-resources`

Harvested from Toddington International free OSINT resources directory (category: Specialty Search). Curated third-party tool reviewed by TII; availability and pricing not independently verified.

_Enrichment: stub. If stub, complete per `schema/templates/tool.template.md`._
